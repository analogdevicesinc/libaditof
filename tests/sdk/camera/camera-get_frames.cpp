...
const std::map<std::string, std::array<ModeDetails_struct, 10>>
    g_modeDetailsMap = {{"crosby",
                         {{
                             {1024, 1024}, // Mode 0
                             {1024, 1024}, // Mode 1
                             {512, 512},   // Mode 2
    ...
    if (init_status == Status::OK) {
        ...
        for (uint16_t cnt = 0; cnt < g_num_frames; cnt++) {
            ...
            if (frameNumber == 0) {
                ...
                if (metadata.frameNumber != frameNumber + 1) {
                    ADD_FAILURE()
                        << "Expected Frame Number: " << frameNumber + 1
                        << " Got: " << metadata.frameNumber;
                }
                frameNumber = metadata.frameNumber;
            }
        ...
        double seconds =
            std::chrono::duration_cast<std::chrono::duration<double>>(endTime -
                ...
                .count();
        double fps =
            seconds > 0.0 ? static_cast<double>(framesReceived) / seconds : 0.0;
        ...
        fps = std::round(fps);
        if (g_num_frames > 10) {
            if (static_cast<int>(fps) < g_fps - 1 ||
                static_cast<int>(fps) >
                    g_fps + 1) { // Allow a margin of error of 1 fps
...
TEST_F(CameraTestFixture, CameraGetFramesAllModes) {
    ...
    if (init_status == Status::OK) {
        ...
        for (const auto &mode : available_modes) {
            ...
            ::testing::Test::RecordProperty("mode_" + std::to_string(g_mode),
                                            g_mode);
            ...
            for (uint16_t cnt = 0; cnt < g_num_frames; cnt++) {
                ...
                frame.getDataDetails("ab", fDetails);
                ASSERT_TRUE(fDetails.width == crosbyModeDetails[g_mode].width);
                ASSERT_TRUE(fDetails.height ==
                            crosbyModeDetails[g_mode].height);
            ...
            double seconds =
                std::chrono::duration_cast<std::chrono::duration<double>>(
                    endTime - startTime)
                    .count();
            double fps = seconds > 0.0
                             ? static_cast<double>(framesReceived) / seconds
                             : 0.0;
            ...
            ::testing::Test::RecordProperty(
                "frames_received_" + std::to_string(g_mode), framesReceived);
            ::testing::Test::RecordProperty(
                "elapsed_seconds_" + std::to_string(g_mode), seconds);
            ::testing::Test::RecordProperty("fps_" + std::to_string(g_mode),
                                            fps);
            fps = std::round(fps);
            if (g_num_frames > 10) {
                if (static_cast<int>(fps) < g_fps - 1 ||
...
int main(int argc, char **argv) {
    ...
    runner.addArgument({"--module=", &g_module,
                        "Specify the camera module (default: crosby)"});
    runner.addArgument(
        {"--mode=", &g_mode, "Specify the camera mode (default: 1)"});
    runner.addArgument(
        {"--frames=", &g_num_frames,
         "Specify the number of frames to capture (default: 1)"});
    runner.addArgument(
        {"--fps=", &g_fps, "Specify the frames per second (default: 10)"});
    runner.addArgument({"--save", &g_savelastframe,
                        "Save the last captured frame to a binary file"});
    runner.addArgument({"--order=", &g_modeorder,
                        "Order for modes (deftault: 0 - ascending, 1 - "
                        "descending, 2 - random)"});

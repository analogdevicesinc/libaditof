...
const std::vector<std::string> g_depthparams = {"bitsInPhaseOrDepth",
                                                "bitsInAB",
                                                "bitsInConf",
                                                "confThresh",
...
const std::vector<std::string> g_configurationparams = {"fps", "xyzEnable"};
...
TEST_F(CameraTestFixture, parametercomparedefault) {
    ...
    if (init_status == Status::OK) {
        ...
        result = aditof_test::compareJsonFiles(
            g_moduleJSONMap.at(g_module), jsonFilePathWorking, differences);
        // Print differences if any
        for (const auto &diff : differences) {
            RecordProperty("Difference at '" + diff.first + "'",
                           std::to_string(diff.second.first) + " vs " +
                               std::to_string(diff.second.second));
            std::cout << "Difference at '" << diff.first
                      << "': " << diff.second.first << " vs "
...
TEST_F(CameraTestFixture, parameterchangevalues) {
    ...
    if (init_status == Status::OK) {
        ...
        for (const auto &mode : available_modes) {
            ...
            values.clear();
            result = aditof_test::readJsonParameter(
                jsonFilePathWorking, std::to_string(mode), "depth-compute",
                g_depthparams, values);
            ASSERT_TRUE(result);
            ...
            result = aditof_test::changeJsonParameter(jsonFilePathWorking,
                                                      std::to_string(mode),
                                                      "depth-compute", values);
            ASSERT_TRUE(result);

            values.clear();
            result = aditof_test::readJsonParameter(
                jsonFilePathWorking, std::to_string(mode),
                "configuration-parameters", g_configurationparams, values);
            ASSERT_TRUE(result);
            // Modify each parameter by adding 1.0
            for (auto &param : values) {
                param.second += 1.0;
            }
            result = aditof_test::changeJsonParameter(
                jsonFilePathWorking, std::to_string(mode),
        ...
        std::map<std::string, std::pair<double, double>> differences;
        aditof_test::compareJsonFiles(jsonFilePathWorking,
                                      jsonFilePathReference, differences);
        ASSERT_TRUE(result == true);
        // Print differences if any
        for (const auto &diff : differences) {
...
int main(int argc, char **argv) {
    ...
    for (const auto &module : g_moduleJSONMap) {
        g_moduleJSONMap.at(module.first) =
            fullPath + "/" + g_moduleJSONMap.at(module.first);
    }
